...
getData() {
    ...
    if [[ "$can_change" != "yes" ]]; then
        echo " 部署Trojan-Go前请确认已具备以下条件："
        echo -e "  1. 一个${RED}伪装域名${PLAIN}"
        echo -e "  2. 伪装域名${RED}DNS解析到${IP}${PLAIN} (若/root目录下已有 ${GREEN}trojan-go.pem${PLAIN} 和 ${GREEN}trojan-go.key${PLAIN} 文件，可无视此条件)"
        echo " "
        read -p " 确认满足输入y，输入其他退出脚本：" answer
        if [[ "${answer,,}" != "y" ]]; then
            exit 0
        fi
        ...
        while true
        do
            read -p " 请输入伪装域名 (例如: baidu.com)：" DOMAIN
            if [[ -z "${DOMAIN}" ]]; then
                echo -e " ${RED}伪装域名输入有误，请重新输入！${PLAIN}"
            else
                break
            fi
        done
        ...
        if [[ -f ~/trojan-go.pem && -f ~/trojan-go.key ]]; then
            echo -e "${GREEN} 检测到已有域名证书，将用于部署${PLAIN}"
    ...
    echo ""
    read -p " 请设置trojan-go密码(留空则随机生成):" PASSWORD
    [[ -z "$PASSWORD" ]] && PASSWORD=`cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 16 | head -n 1`
    colorEcho $BLUE " 密码：$PASSWORD"
    echo ""

    read -p " 请输入Trojan-Go端口 [从100-65535中选择一个数字，留空则为443]：" PORT
    [[ -z "${PORT}" ]] && PORT=443
    if [[ "${PORT:0:1}" = "0" ]]; then
        echo -e "${RED}端口数字不能以0开头${PLAIN}"
        exit 1
    fi
    ...
    if [[ -z "$answer" ]]; then
        ...
        2)
            ...
            ;;
        3)
            read -p " 请输入反代站点 (以http或https开头)：" PROXY_URL
            if [[ -z "$PROXY_URL" ]]; then
    ...
    echo "    (y)允许。这可以增强伪装能力，VPS流量充足时建议允许"
    echo "    (n)不允许。稍微降低了伪装能力，但能节省VPS流量"
    ...
    echo ""
    colorEcho $BLUE " 是否允许爬虫：$ALLOW_SPIDER"

    echo ""
    read -p " 是否安装BBR? (默认安装) [y/n]:" NEED_BBR
    [[ -z "$NEED_BBR" ]] && NEED_BBR=y
    [[ "$NEED_BBR" = "Y" ]] && NEED_BBR=y
    colorEcho $BLUE " 是否安装BBR：$NEED_BBR"
}
...
module_hotfixes=true' > /etc/yum.repos.d/nginx.repo
        ...
        if [[ "$?" != "0" ]]; then
            colorEcho $RED " 您安装了宝塔面板，请在宝塔后台安装nginx后再运行本脚本"
            exit 1
        fi
    fi
}
...
getCert() {
    echo ""
    colorEcho $BLUE " 获取Let's Encrypt证书..."
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        if [[ "${res}" != "" ]]; then
            echo -e "${RED} 其他进程占用了80或443端口，请先关闭这些进程再运行脚本${PLAIN}"
            echo " 端口占用信息如下："
            echo ${res}
            exit 1
        fi
        ...
        curl -sL https://get.acme.sh | sh -s email=GFW@magicGFW.com
        source ~/.bashrc
        ~/.acme.sh/acme.sh  --upgrade  --auto-upgrade
        ~/.acme.sh/acme.sh --set-default-ca --server letsencrypt
        if [[ "$BT" = "false" ]]; then
            ~/.acme.sh/acme.sh  --insecure --issue -d $DOMAIN --keylength ec-256 --pre-hook "systemctl stop nginx" --post-hook "systemctl restart nginx"  --standalone
        else
            ~/.acme.sh/acme.sh  --insecure --issue -d $DOMAIN --keylength ec-256 --pre-hook "nginx -s stop || { echo -n ''; }" --post-hook "nginx -c /www/server/nginx/conf/nginx.conf || { echo -n ''; }"  --standalone
        fi
...
bbrReboot() {
    if [[ "${INSTALL_BBR}" == "true" ]]; then
        echo  
        echo -e " 安装已完成，为使BBR模块生效，请输入 ${RED}reboot${PLAIN} 重启系统"
    fi
}
...
menu() {
    ...
    echo -e "  ${GREEN}9.  ${RED}修改Trojan-Go${PLAIN}"
